                                       ...
                                       )
{
    ...
    {
        bool (*const converter)(string_segment value, void* p_out) = cfg_element->converter_fn;
        if (!converter)
        {
            RMOD_ERROR("Configuration component \"%s\" specified no converter function", cfg_element->name);
            return RMOD_RESULT_WAS_NULL;
        }
        void* const p_out = cfg_element->p_out;
        if (!p_out)
        {
            RMOD_ERROR("Configuration component \"%s\" specified no output ptr", cfg_element->name);
            return RMOD_RESULT_WAS_NULL;
        }
        if (!converter(xml_element->value, p_out))
        {
            RMOD_ERROR("Conversion function of configuration component \"%s\" returned false, indicating failed conversion", cfg_element->name);
